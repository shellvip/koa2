...
router.post('/login', mw.ensureRecaptcha(), async (ctx) => {
  ...
  const user = await db.checkPassword(ctx.vals.uname, ctx.vals.password)//getUserByUname(ctx.vals.uname)
  ctx.check(user, 'Invalid creds')
  // ctx.check(await belt.checkPassword(ctx.vals.password, user.digest), 'Invalid creds')

  // User authenticated

  const session = await db.insertSession(user._id, ctx.ip, ctx.headers['user-agent'], ctx.vals['remember-me'] ? 1000 * 60 * 60 * 24 * 365: 1000 * 60 * 60 * 24 * 14)
...
router.post('/users', mw.ensureRecaptcha(), async (ctx) => {
  ...
  const session = await db.insertSession(user._id, ctx.ip, ctx.headers['user-agent'], 1000 * 60 * 60 * 24 * 365)
...
router.del('/sessions/:id', async (ctx) => {
  ...
  await db.logoutSession(ctx.currUser._id, ctx.vals.id)
